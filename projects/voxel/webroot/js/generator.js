 ...
 * Color functions
 ******************************************************************************/
function calculatePalette(mapping) {
    // Convert to ABGR
...
async function generateHalo() {
    ...
    const mapData = generateMap(landscape, "landscape");
    mapData.shape = -0.5 /* Shape.Concave */;
    mapData.sun = "#14051A";
 ...
 ******************************************************************************/
function generateMap(config, name) {
...
function generateTrees({ altitudes, colors, dimension }, minTreeAltitude, maxTreeAltitude, treeDensity) {
    const trees = [];
...
function generateShadow({ altitudes, colors, dimension }) {
    for (let x = 0; x < dimension; x++) {
        for (let y = 0; y < dimension; y++) {
            const i = x + y * dimension;
            let color = colors[i];
...
async function generateWater({ colors, altitudes, dimension }, waterLevel, withBoat = false) {
...
async function generateTower(mapX, mapY, mapZ, { colors, altitudes, dimension }) {
